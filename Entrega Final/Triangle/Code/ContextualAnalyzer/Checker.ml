(*
This program was made by Luis Leopoldo Pérez on March 12, 2006.
This program was repaired, completed, verified and validated by students
of ITCR in January 2019.
...
(**
Reports an undeclared identifier error

@param a  AST type variable
@return ()
*)
...
(**
Reports an undeclared operator error
...
@param id  string, name of the identifier
@return string
*)
...
@param id  string, name of the operator
@return string
*)
...
@param ft ast_field_type_denoter, the record where the field will be selected
@param id  string, name of the field identifier (within the record)
@return type (of the field identifier within a record)
*)
...
(**
  Checks the program, returning a "decorated" abstract syntax tree
  @param a AST, abstract syntax tree
  @return AST, decorated abstract syntax tree
*)
...
(**
Checks whether a Command satisfies Triangle's contextual constraints

@param a AST, Command abstract syntax tree
@return AST, Command "decorated" abstract syntax tree
*)
...
(**
Checks whether an Expression has a type and satisfies Triangle's contextual constraints

@param e AST, Expresion abstract syntax tree
@return AST, a Checked_expression, which is an Expression AST together with its type
*)
...
(**
Checks whether an Array aggregate has a type, its number of members and it satisfies Triangle's contextual constraints

@param a AST, Array aggregate abstract syntax tree
@return AST, a Checked_array_aggregate, which is an Array aggregate AST (that has a type) together with its numbert of members
*)
...
(**
Checks whether a Record aggregate has a type and satisfies Triangle's contextual constraints

@param r AST, Record aggregate abstract syntax tree
@return AST, a Checked_record_aggregate, which is a Record aggregate AST together with its type
*)
...
(**
Determines the nature of a Value or variable name: constant, variable, constant parameter, variable parameter, or error

@param v AST, Value or variable name abstract syntax tree
@return AST, a Checked_vname, which is a vName AST together with its mode (constant, variable) and its type
*)
          ...
          (match t with
            Record_type_denoter(_,ft) -> 
              let fType = (visit_field_identifier ft id) in
                (match fType with
                  Error_type_denoter(_) -> 
                    ErrorReporter.report_error ("No field " ^ (identifier_name id) ^ " in this record type") ix.pos; v
                | _ -> 
                    Checked_vname(Dot_vname(ix,vType,(check_identifier id)), var, false, 0,fType)
                )
          | _ -> ErrorReporter.report_error "Record expected here" ix.pos; v
          )
      | _ -> v
      )
        ...
        Checked_vname(_,var,_,_,t) -> 
          (match t with
            Array_type_denoter(_,_,tt) -> 
              ...
              | _ -> 
                  ErrorReporter.report_error "Integer expression expected here" ix.pos; v
              )
          | _ -> ErrorReporter.report_error "Array expected here" ix.pos; v
          )
      | _ -> v
      )
...
(**
Processes a Declaration in order to enrich the environment (context)

@param d AST, Declaration abstract syntax tree
@effects Enters identifiers into the Identification table (which is the environment)
@return AST, a 'processed' AST
*)
...
(**
Performs the matching of type and mode of actual and formal parameters

@param a AST, Actual parameter AST
@param f AST, Formal parameter AST
@return AST, a parameter AST - the type of the binding between formal an actual parameters
*)
...
| Proc_actual_parameter(ix,id) -> 
    let idType = (check_identifier id) in
      ...
      | _ -> ErrorReporter.report_error "Proc actual parameter not expected here" ix.pos; a
      ) 
...
| Func_actual_parameter(ix,id) -> 
    let idType = (check_identifier id) in
      ...
      | _ -> ErrorReporter.report_error "Func actual parameter not expected here" ix.pos; a
      )
...
(**
Performs the matching of type and mode of actual and formal parameter sequences

@param a AST, Actual parameter sequence AST
@param f AST, Formal parameter sequence AST
@return AST, an Actual parameter sequence AST - the types of the bindings between formal an actual parameters
*)
and check_actual_parameter_sequence a f = match a with
  Empty_actual_parameter_sequence(ix) -> 
    ...
    | _ -> ErrorReporter.report_error "Too few actual parameters" ix.pos; a
    )
...
| Single_actual_parameter_sequence(ix,b) -> 
    ...
    | _ -> ErrorReporter.report_error "Incorrect number of actual parameters" ix.pos; a
    )
...
| Single_formal_parameter_sequence(ix,fp) -> 
    Single_formal_parameter_sequence(ix, checkFormalParameter fp)
  
| Multiple_formal_parameter_sequence(ix,fp,fps) ->
    let fpType = (checkFormalParameter fp)
...
| Simple_type_denoter(ix,i) -> 
    let b = (check_identifier i) in
      (match b with
        Checked_identifier(_,a) -> 
          (match !a with
            Null_declaration -> 
			  report_undeclared_identifier b;
              Error_type_denoter(ix)
          | Type_declaration(_,_,t) -> t
          | _ -> 
		      ErrorReporter.report_error ((identifier_name i) ^ " is not a type Identifier") ix.pos;
			  Error_type_denoter(ix)
	      )
      | _ -> Error_type_denoter(ix)
      )
...
| (Array_type_denoter(_,il1,tx1),Array_type_denoter(_,il2,tx2)) -> 
    let size =  
      (match (il1,il2) with 
	    (Integer_literal(_,s1),Integer_literal(_,s2)) -> 
...
| (Var_formal_parameter(_,_,t1),Var_formal_parameter(_,_,t2)) -> 
    (compare_types t1 t2)
| (Proc_formal_parameter(_,_,fps1),Proc_formal_parameter(_,_,fps2)) -> 
    (compare_fps fps1 fps2)
| (Func_formal_parameter(_,_,fps1,t1),Func_formal_parameter(_,_,fps2,t2)) -> 
    (compare_fps fps1 fps2) && (compare_types t1 t2)
| _ -> false
...
| (Single_formal_parameter_sequence(_,fp1),Single_formal_parameter_sequence(_,fp2)) -> 
    (compare_fp fp1 fp2)
| (Multiple_formal_parameter_sequence(_,fp1,fps1),Multiple_formal_parameter_sequence(_,fp2,fps2)) -> 
    (compare_fp fp1 fp2) && (compare_fps fps1 fps2)
| _ -> false  