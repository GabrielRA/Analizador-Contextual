...
and check_command a = match a with
  ...
  | Call_command(ix, i, aps)       -> let iType = (check_identifier i) in
                                         (match iType with
                                           Checked_identifier(_,d)  -> (match !d with
                                                                       ...
                                                                       | _                                                           -> ErrorReporter.reportError ((Identifier_name i) ^ " is not a procedure Identifier") ix.pos;
                                                                                                                                        a)
                                         | _                       -> a)
  ...
  | Let_command(ix, d, c)          -> IdentificationTable.openScope();
                                     let dType = (check_declaration d)
                                     and cType = (check_command c) in
                                         IdentificationTable.closeScope();
                                         Let_command(ix, dType, cType)
  
    (* If command - checks if the expression is boolean, then checks both limbs recursively *)
  | If_command(ix, e, c1, c2)      -> let eType = (check_expression e) in
                                         (match eType with
...
and check_expression e = match e with
  ...
  | Call_expression(ix, i, ap)       -> let iType = (check_identifier i) in
                                           (match iType with
                                              Checked_identifier(_,d) -> (match !d with
                                                                           Null_declaration                                              -> report_undeclared_identifier i;
                                                                                                                                           e
                                                                         ...
                                                                         | _                                                            -> ErrorReporter.reportError ((Identifier_name i) ^ " is not a function Identifier") ix.pos;
                                                                                                                                           e)
  ...
  | Let_expression(ix, d, ex)        -> IdentificationTable.openScope();
                                       let dType = (check_declaration d)
                                       and eType = (check_expression ex) in
                                           IdentificationTable.closeScope();
                                           (match eType with
  ...
  | Unary_expression(ix, o, ex)      -> let eType = (check_expression ex)
                                       and oType = (check_operator o) in
                                           (match oType with
                                              Checked_operator(_,d) -> (match !d with
                                                                         Null_declaration                      -> report_undeclared_operator o;
                                                                                                                 Checked_expression(e, Error_type_denoter(ix))
                                                                       | Unary_operator_declaration(_,o,t,tr)   -> (match eType with
                                                                                                                  ...
                                                                                                                  | _                       -> e)
                                                                       | _                                    -> ErrorReporter.reportError ((operator_name o) ^ " is not an unary operator") ix.pos;
                                                                                                                 e)
                                            | _                    -> e)
                                       ...
                                       and oType  = (check_operator o) in
                                           (match oType with
                                              Checked_operator(_,d) -> (match !d with
                                                                       ...
                                                                       | _                                       -> ErrorReporter.reportError ((operator_name o) ^ " is not a binary operator") ix.pos;
                                                                                                                    e)
                                            | _                    -> e)
  
  
    (* Array expression - returns an array type denoter *)
  | Array_expression(ix, aa)         -> let aaType = (check_array_aggregate aa) in
                                           (match aaType with
                                              Checked_array_aggregate(Single_array_aggregate(_,Checked_expression(_,t)),i)
                                            | Checked_array_aggregate(Multiple_array_aggregate(_,Checked_expression(_,t),_),i) -> Checked_expression(Array_expression(ix,aaType),Array_type_denoter(ix,Integer_literal(ix,string_of_int i),t))
                                            | _                                                                           -> e)
                                            
    (* Record expression - returns a record type denoter *)
  | Record_expression(ix, ra)        -> let raType = (check_record_aggregate ra) in
                                           (match raType with
...
and check_array_aggregate a = match a with
  ...
  | Multiple_array_aggregate(ix,e,aa) -> let eType = (check_expression e) 
                                       and aaType = (check_array_aggregate aa) in
                                           (match eType with
                                              Checked_expression(_,t) -> (match aaType with
...
and check_record_aggregate r = match r with
    Single_record_aggregate(ix,i,e)      -> let eType = (check_expression e) in
                                              (match eType with
                                                 Checked_expression(_,t) -> Checked_record_aggregate(Single_record_aggregate(ix,i,eType),Single_field_type_denoter(ix,i,t))
                                               | _                      -> r)
                                               
  | Multiple_record_aggregate(ix,i,e,r)  -> let eType = (check_expression e)
                                          and rType = (check_record_aggregate r) in
                                              (match rType with
                                                 Checked_record_aggregate(_,t) -> let fType = (visit_field_identifier t i) in
                                                                                    (match fType with
                                                                                       Error_type_denoter(_) -> ()
                                                                                     | _                   -> ErrorReporter.reportError ("Duplicate field " ^ (Identifier_name i) ^ " in record") ix.pos);
                                                                                    Checked_record_aggregate(Multiple_record_aggregate(ix, i, eType, rType), Multiple_field_type_denoter(ix, i, (match eType with Checked_expression(_,tt) -> tt | _ -> Error_type_denoter(ix)), t))
                                               | _                           -> r)
...
and check_declaration d = match d with
  ...
  | Const_declaration(ix,i,e)                 -> let eType = (check_expression e) in
  ...
  | Var_declaration(ix,i,t)                   -> let tType = (check_type_denoter t) in
                                                    if (IdentificationTable.exists (Identifier_name i)) then
                                                       ErrorReporter.reportError ("Identifier " ^ (Identifier_name i) ^ " already declared") ix.pos;
  ...
  | Func_declaration(ix,i,fps,t,e)            -> let tType = (check_type_denoter t) in
                                                    if (IdentificationTable.exists (Identifier_name i)) then
                                                       ErrorReporter.reportError ("Identifier " ^ (Identifier_name i) ^ " already declared") ix.pos;
                                                    ...
                                                    let elem = IdentificationTable.retrieveElement (Identifier_name i) in
                                                        ...
                                                        let fpsType = (check_formal_parameter_sequence fps) in
                                                            ...
                                                            let eType = (check_expression e) in
                                                                ...
                                                                (match eType with
                                                                   Checked_expression(_,t) -> if (t != tType) then
...
and check_actual_parameter a f = match a with
    Const_actual_parameter(ix,e) -> let eType = (check_expression e) in
                                      (match f with
                                         Const_formal_parameter(_,_,t) -> (match eType with
                                                                           Checked_expression(_,tt) -> if ((compare_types t tt) == false) then
                                                                                                      begin
  ...
  | Proc_actual_parameter(ix,id) -> let idType = (check_identifier id) in
                                     (match f with
                                        Proc_formal_parameter(_,_,fp)  -> (match idType with
                                                                           Checked_identifier(i,d) -> (match !d with
                                                                                                        Null_declaration                                            -> report_undeclared_identifier(i);
                                                                                                                                                                      a
                                                                                                      | Formal_parameter_declaration(_,Proc_formal_parameter(_,i,fps))
                                                                                                      | Proc_declaration(_,i,fps,_)                                 -> if ((compare_fps fp fps) == false) then
  ...
  | Func_actual_parameter(ix,id) -> let idType = (check_identifier id) in
                                      (match f with
                                         Func_formal_parameter(_,_,fp,t) -> (match idType with
                                                                             Checked_identifier(i,d) -> (match !d with
                                                                                                          Null_declaration               -> report_undeclared_identifier(i);
                                                                                                                                           a
                                                                                                        | Formal_parameter_declaration(_,Func_formal_parameter(_,i,fps,tp))
                                                                                                        | Func_declaration(_,i,fps,tp,_) -> if ((compare_fps fp fps) == false) then
...
and check_actual_parameter_sequence a f = match a with
  ...
  | MultipleActualParameterSequence(ix,b,c) -> (match f with
                                                    Multiple_formal_parameter_sequence(ixx,fp,fps) -> let fpType = (check_actual_parameter b fp)
                                                                                                   and fpsType = (check_actual_parameter_sequence c fps) in
...
and checkFormalParameter f = match f with
    Const_formal_parameter(ix,i,t)    -> let tType = (check_type_denoter t) in
                                       if (IdentificationTable.exists (Identifier_name i)) then
  ...
  | Var_formal_parameter(ix,i,t)      -> let tType = (check_type_denoter t) in
                                       if (IdentificationTable.exists (Identifier_name i)) then
                                           ErrorReporter.reportError ("Duplicated formal parameter " ^ (Identifier_name i)) ix.pos;
  ...
  | Proc_formal_parameter(ix,i,fps)   -> IdentificationTable.openScope();
                                       let fpsType = (check_formal_parameter_sequence fps) in
                                           IdentificationTable.closeScope();
                                       if (IdentificationTable.exists (Identifier_name i)) then
                                           ErrorReporter.reportError ("Duplicated formal parameter " ^ (Identifier_name i)) ix.pos;
  ...
  | Func_formal_parameter(ix,i,fps,t) -> IdentificationTable.openScope();
...
and check_field_type_denoter a = match a with
  ...
  | Multiple_field_type_denoter(ix,i,t,ft) -> let tType = (check_type_denoter t)
                                           and ftType = (check_field_type_denoter ft) in
...
and compare_types t1 t2 = match (t1,t2) with
  ...
  | (Record_type_denoter(_,ft1),Record_type_denoter(_,ft2)) -> let rec compareFTs f1 f2 = (match (f1,f2) with
                                                             ...
                                                             | (Multiple_field_type_denoter(_,i1,tx1,ftx1),Multiple_field_type_denoter(_,i2,tx2,ftx2)) -> (compare_types tx1 tx2) && ((String.compare (Identifier_name i1) (Identifier_name i2)) == 0) && (compareFTs ftx1 ftx2)
                                                             | _                                                                                 -> false) 
                                                             in
                                                               (compareFTs ft1 ft2)
